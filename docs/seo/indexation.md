...
| `/kind/:kind`                       | ✅ (page 1)          | "Skills for DeepSeek Harness" is a phrase people type. Retired `/kind/mcp-server` and `/kind/hook-bridge` 301 onto `/browse`.                                                  |
...
| `/docs/…` (MDX pages)               | ✅                   | One URL per physical locale guide (`/docs/cli`, `/docs/publish/skill`, …). Retired `/docs/publish/mcp-server` and `/docs/publish/hook-bridge` 301 onto `/docs/plugins`. Generated from the Fumadocs source into `pages.xml`; fallback-only locales are not advertised. |
| `/docs/search`                      | ❌                   | JSON title/description index for the section. Not a document.                                                                                                                   |
| `/blog`                             | ✅                   | The editorial index. Four series, newest first.                                                                                                                                 |
| `/blog/{series}`                    | ✅                   | Series landing (`harness`, `deepseek`, `changelog`, `notes`).                                                                                                                   |
| `/blog/{series}/{slug}`             | ✅                   | One URL per physical locale post. Generated from the Fumadocs blog collection into `pages.xml`; fallback-only locales are not advertised.                                      |
| `/blog/feed.xml`                    | ❌                   | Atom of every post. Discovery, not a document.                                                                                                                                  |
| `/blog/llms.txt`                    | ❌                   | Agent overview of the blog tree.                                                                                                                                                |
