...
is included, not only the popular or recently updated rows. One entry per path
keeps the files small; 2,500 rows per file stays far below the 50 MB
uncompressed limit with room for longer ids.
...
Each path appears once. There is no `xhtml:link` alternate set: `hreflang`
connects distinct per-language URLs, and one negotiated URL serves every
language.
...
artifacts, in the negotiated language like every reader-facing page.
...
Every entry's `<id>` is the artifact's one canonical URL, so the same
artifact is the same entry no matter which language the feed was negotiated
into, and a subscriber switching languages does not see the whole catalog as
new. Each indexable page advertises the feed with a
`<link rel="alternate" type="application/atom+xml">` in the head, emitted by
`pageMeta` next to the canonical.
...
Language variants are not in the internal link graph because there are none:
one URL serves every language. The header language switcher records an
explicit choice in a cookie; it is a reader control, not a navigation one.
...
The site card deliberately contains no translatable sentence. Every language
shares the same image while the `og:title`, `og:description`,
`og:locale`, and image alt text stay localized in the page head; an English-only
claim baked into the bitmap would contradict most of those previews.
