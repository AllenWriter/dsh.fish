...
├── /sitemaps/pages.xml            home, blog, docs, browse, kinds, categories, every /docs slug, every /blog slug, submit
...
three locales, production XML is smaller per artifact after its alternate
...
For a path translated into all public languages that is 3 entries × 4 links.
...
language finds the other public languages through the `hreflang` set in the page head and
...
The site card deliberately contains no translatable sentence. All public language
