 ...
 *
 * `domcontentloaded` returns while the page is still the server's HTML, and the
 * controls this suite clicks — the theme toggle, the menu toggle — only know their
 * state after an effect. The account slot is the signal: it renders a blank
 * placeholder until the session resolves in the browser, so a sign-in link in the
 * bar means hydration is done.
 */
