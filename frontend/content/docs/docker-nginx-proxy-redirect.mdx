...
description: "服务换机器之后，用一个 Nginx 容器把旧端口 3 秒跳到新地址。"
---

Docker 真是一个非常牛逼的工具，以至于许多服务都是通过 Docker 占用一个端口号，然后再通过端口映射 + [Nginx Proxy Manager](/docs/nginx-proxy-manager) 就可以提供服务了，不需要再折腾复杂的 Nginx 或者宝塔什么之类的平台，一行命令，一行配置就能完事。
