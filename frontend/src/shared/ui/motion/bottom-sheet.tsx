...
import { EASE_DRAWER } from "@/shared/lib/ease";
import { TOUCH_GESTURE_CONTENT_CLASS } from "@/shared/lib/touch";
...
export function BottomSheet({
  ...
  const onDragEnd = (_: unknown, info: PanInfo) => {
    ...
    if (velocity > 600 || offset > dismissThreshold) {
      ...
      const nextSnap = smaller[smaller.length - 1];
      if (nextSnap !== undefined && velocity < 800 && offset < dismissThreshold * 1.6) {
        setSnap(nextSnap);
      } else {
        onOpenChange(false);
      }
  ...
  const snapValue = snapPoints[snap] ?? snapPoints[0] ?? 0.72;
  const heightStyle =
    snapValue === "auto"
      ? { maxHeight: "92vh" }
