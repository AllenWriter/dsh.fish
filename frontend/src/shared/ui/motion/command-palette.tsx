// beui.dev/components/blocks/command-palette — vendored from the beui registry.
...
export interface CommandPaletteProps {
  ...
  /** Called on every query change, for live server-backed results. */
...
function fuzzyMatch(needle: string, hay: string) {
  ...
  let i = 0
  for (const ch of lowerHay) {
    if (ch === lowerNeedle[i]) i++
    if (i === lowerNeedle.length) return true
  }
  return false
}
