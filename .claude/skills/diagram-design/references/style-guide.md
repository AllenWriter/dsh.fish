...
Active profile: dsh-fish-local

This project skin is extracted from `frontend/src/app/styles/app.css`: cool near-neutral paper, one whale-blue accent, and IBM Plex typography. New diagrams inherit these values without introducing a second accent hue.
...
|---|---|---|---|
| `paper` | Page background, default node fill | `#faf9fc` | `#171b25` |
| `paper-2` | Diagram container bg, secondary fill | `#f1eff7` | `#242936` |
| `ink` | Primary text, primary stroke | `#17151d` | `#f3f4f6` |
| `muted` | Secondary text, default arrow stroke | `#68636f` | `#a8acb6` |
| `soft` | Sublabels, boundary labels | `#807b88` | `#8e94a1` |
| `rule` | Hairline borders | `rgba(23,21,29,0.12)` | `rgba(243,244,246,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#d8d4df` | `rgba(243,244,246,0.25)` |
| `accent` | Focal / 1–2 max per diagram | `#7867d9` | `#a99dea` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(120,103,217,0.10)` | `rgba(169,157,234,0.12)` |
| `link` | HTTP/API calls, external arrows | `#4e5fc7` | `#93a0f0` |

> **Brand palette source:** `frontend/src/app/styles/app.css`. The light tokens map the project's `--bg`, `--fg`, `--muted-fg`, `--line`, and hue-263 `--primary` roles to SVG-safe sRGB values. The one-accent rule matches the product UI.
...
|---|---|---|---|---|
| `title` | IBM Plex Sans | 1.75rem | 600 | Page H1 |
